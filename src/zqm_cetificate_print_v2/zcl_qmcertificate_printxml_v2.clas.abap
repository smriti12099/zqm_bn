 CLASS zcl_qmcertificate_printxml_v2 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    INTERFACES if_oo_adt_classrun .
     ...
     CLASS-METHODS :
       ...
       read_posts
         IMPORTING lv_insp_num     TYPE string
...
CLASS ZCL_QMCERTIFICATE_PRINTXML_V2 IMPLEMENTATION.
   ...
   METHOD read_posts. "if_oo_adt_classrun~main.
     ...
     SELECT SINGLE FROM i_inspectionlot  with PRIVILEGED ACCESS   AS a
     LEFT JOIN i_supplier  with PRIVILEGED ACCESS AS b ON a~supplier = b~supplier
     LEFT JOIN i_inspectionresultvalue with PRIVILEGED ACCESS AS c ON a~inspectionlot = c~inspectionlot
     LEFT JOIN i_user with PRIVILEGED ACCESS AS d ON c~CreatedByUser = d~UserID
     LEFT JOIN i_insplotusagedecision with PRIVILEGED ACCESS AS e ON e~InspectionLot = a~InspectionLot
     LEFT JOIN I_UsageDecisionCodeText with PRIVILEGED ACCESS AS f ON f~UsageDecisionCode = e~InspectionLotUsageDecisionCode and f~UsageDecisionCodeGroup = e~InspLotUsageDecisionCodeGroup
     FIELDS
       a~inspectionlotobjecttext, a~material, a~inspectionlot, a~materialdocument, a~inspectionlotquantity, a~PurchasingOrganization,
       a~insplotcreatedonlocaldate, a~insplotqtytoblocked, a~inspectionlotsamplequantity, a~companycode,
       b~suppliername, c~inspector , d~UserDescription , f~UsageDecisionCodeText
     WHERE a~inspectionlot = @lv_insp_num
     INTO @DATA(wa_head).

     " Fetch item data
     SELECT FROM i_inspectionlot AS a
       LEFT JOIN i_inspectioncharacteristic with PRIVILEGED ACCESS AS b ON a~inspectionlot = b~inspectionlot
       LEFT JOIN i_inspectionresultvalue  with PRIVILEGED ACCESS AS c ON b~inspectionlot = c~inspectionlot AND b~inspectioncharacteristic = c~inspectioncharacteristic
       LEFT JOIN i_inspectioncodetext  with PRIVILEGED ACCESS AS d ON d~inspectioncodegroup = c~CharacteristicAttributeCodeGrp AND d~inspectioncode = c~characteristicattributecode
       FIELDS
...
* OUT->WRITE( it_line ).
     ...
     DATA(lv_xml) = |<Form>| &&
                    ...
                    |<CompanyCode>{ wa_head-PurchasingOrganization }</CompanyCode>| &&
                    |<SupplierName>{ wa_head-suppliername }</SupplierName>| &&
                    |<codeOfInspector>{ wa_head-inspector }</codeOfInspector>| &&
                    |<NameOfInspector>{ wa_head-UserDescription }</NameOfInspector>| &&
                    |<passreject>{ wa_head-UsageDecisionCodeText }</passreject>| &&
                    |</Inspection>| &&
                    |<item>|.
